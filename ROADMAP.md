...
- [ ] **Structured talent effects** — annotate each talent in `talents.json` with a discriminated-union `effects` array (`derivedStat`, `attribute`, `skillModifier`, `action`, `resource`, `weaponMod`, `note`). Layer `applyPassiveTalentEffects()` on top of `computeAllDerivedStats` so passive bonuses (Iron Skin: +1 soak / +4 max HP, Training: X: +1 attribute, Sprinter, etc.) reflect on the sheet automatically. Conditional / triggered effects feed the action-card panel.
